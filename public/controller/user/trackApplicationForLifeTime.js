...
app.controller("myCtr", function ($scope, $http, $filter) {
    ...
    $scope.track = function (y) {
        $http.get("http://localhost:1000/user/trackPageForLifeTime?userId=" + userId)
    ...
    $scope.downloadCertificate = (data) => {
        window.location.href = "http://localhost:1000/user/ilpCertificate?regNo=" + data.registrationNo;
    }
