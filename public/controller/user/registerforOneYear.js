...
app.controller("myCtr", function ($scope, $http) {
        ...
        if (file != undefined) {
            switch (fileName) {
                case 'proofOfRelationship': {
                    $scope.proofOfRelationshipList.push(file);
    ...
    $scope.removeCertificate = (fileName, index) => {
        switch (fileName) {
            case 'proofOfRelationship': {
                $scope.proofOfRelationshipList.splice(index);
                break;
            }
        ...
        if ($scope.applyNo != '') {
            $http.get("http://localhost:1000/user/getApplicationDetails?applyNo=" + $scope.applyNo)
                .then(function (response) {
                    ...
                    $scope.docSubmitBy = $scope.details.docSubmitBy;
                    ...
                    if ($scope.details.proofOfRelationshipUrl != undefined) {
                        $scope.showProofOfRelationshipError = true;
                        $scope.proofOfRelationshipFileGood = "UPLOADED";
                        $scope.proofOfRelationship = true;
                    }
    ...
    $scope.preview = () => {
        ...
        if (filledAppForm != undefined && nativeAddressProof != undefined && govtPhotoId != undefined && userPhoto != undefined) {
    ...
    $scope.submit = function () {
        ...
        if (filledAppForm != undefined && nativeAddressProof != undefined && govtPhotoId != undefined && userPhoto != undefined) {
            ...
            $scope.data.docSubmitBy = $scope.docSubmitBy;
            ...
            data.append('proofOfRelationship', document.querySelector('#proofOfRelationship').files[0]);
            $scope.proofOfRelationshipList.forEach(element => {
                data.append('proofOfRelationship', element)
            }) 
            ...
            request.onreadystatechange = function () {
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    window.location.href = "http://localhost:1000/user/registerforOneYear";
                    // window.location.href = "http://localhost:1000/user/registerFinalPage?applyNo=" + this.responseText;
            ...
            request.open('POST', "http://localhost:1000/user/signupForOneYear");
            request.send(data);
    ...
    $scope.update = function () {
        ...
        $scope.data = {};
        $scope.data.tokenNo = $scope.tokenNo;
        $scope.data.ntitle = $scope.nametitle;
        $scope.data.name = $scope.name;
        $scope.data.fatherTitle = $scope.fatherTitle;
        $scope.data.fatherName = $scope.fathername;
        ...
        $scope.data.presentAddress.villege = $scope.psvillege;
        $scope.data.presentAddress.policeStation = $scope.pspolicestation;
        $scope.data.presentAddress.postOffice = $scope.ppostoffice;
        $scope.data.presentAddress.nearby = $scope.pslandmark;
        $scope.data.presentAddress.plotno = $scope.psplotno;
        $scope.data.presentAddress.block_lane = $scope.psblock_lane;
        $scope.data.presentAddress.colony = $scope.pscolony;
        ...
        $scope.data.occupication = $scope.occupication;
        $scope.data.religion = $scope.religion;
        // $scope.data.community = $scope.community;
