...
app.controller("myCtr", function ($scope, $http, $window) {
    ...
    $scope.signup = function (y) {  
        $http.get("http://localhost:1000/va/allPendingData?applyNo="+applyNo)
    ...
    $scope.back = (y) => {
        if(y == 'oneYear')
        {
            window.location.href = "http://localhost:1000/va/verifiedLicencesforOneYear";
        }
        else
        {
            window.location.href = "http://localhost:1000/va/verifiedLicencesforLifeTime";
        }
