...
app.controller("myCtr", function ($scope, $http, $window) {
    ...
    $scope.allApprovedData = function (y) {
        $http.get("http://localhost:1000/admin/getAllApprovedDataForLifeTime")
    ...
    $scope.loadStates = function () {
        $http.get("http://localhost:1000/admin/state")
            .then(function (response) {
    ...
    $scope.loadFromDistricts = function () {
        if ($scope.fromStateFilter != null) {
            $http.get("http://localhost:1000/admin/dist?stateCode=" + $scope.fromStateFilter.stateCode)
                .then(function (response) {
    ...
    $scope.view = function (y) {
        $scope.allDetails = y;
        window.location.href = "http://localhost:1000/admin/viewPage?applyNo=" + $scope.allDetails.applyNo;
    }
